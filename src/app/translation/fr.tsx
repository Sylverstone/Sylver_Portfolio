...
D'ailleurs ce portfolio a été développé en apprenant le framework Next.Js
		`,
		textePhoto : "Développeur de jeux vidéo",
		passion : "Passionné par le codage",
		notFound : "Que cherchez vous ?",
	},
	...
	project : 
	{
		SylverService : {
			...
			<p>
				...
				Nous avons conçu des <strong>algorithmes efficaces</strong> afin <strong>d'optimiser les fonctionnalités clés de l’application</strong>.<br /> Ce projet a été l’occasion de me <strong>concentrer entièrement</strong> sur le développement d’une application complète, de l’architecture technique à l’interface utilisateur.
				En parallèle, j’ai utilisé <strong>Git et GitHub</strong> de manière intensive pour le versioning et la gestion collaborative du code source.<br />
				L'application utilise une <strong>base de donnée</strong> MySQL hébergé sur Alwaysdata, j'y ai instauré des <strong>triggers</strong> ainsi que des <strong>procédures</strong> pour assurer la cohérence entre les données, ajouter des contraintes à la base.
			</p>
			),
			fonctionnalitees : (
			<>
				...
				<p>
					Je vous laisse en savoir plus sur le site web dédié :). L'application est disponible sur son GitHub.
				</p>
			</>
			)
		},
		BotDiscordBDE : {
			...
			<p>	
				...
				Durant le développement j'ai fait la transition vers <strong>TypeScript</strong> car je trouvais qu'il devenait de plus en plus contraignant de coder en JavaScript à cause du typage. Avec TypeScript j'ai pu <strong>développer plus facilement</strong> et surtout plus sereinement.<br />
			...
			<>
				...
				<p>C'est grâce a une <strong>base de donnée</strong> MySQL que le bot est capable de <strong>faire la distinction</strong> entre les informations des guilds, les enregistrements sont fait avec l'id de la guild, ce qui évite toutes confusions dans les requêtes SQL</p>
...
<				strong>pgmToAA → pgm to AsciiArt</strong> Est un projet mené en équipe de 2 lors d'une SAÉ à l'IUT. Dans celle-ci nous devions implémenter les besoins d'un client qui voulait une application console permettant de convertir une image en AsciiArt. Et pour se faire nous passions par le format .pgm, d'où le titre du projet.
		...
		SylverDonjon : {
            ...
            (
			<p>
				<strong>SylverDonjon</strong> est un projet Python réalisé en <strong>équipe de trois personnes</strong>, présenté au Trophée NSI 2023.
				Nous étions <strong>deux programmeur</strong>s accompagnés d'un <strong>designer</strong> pour ce projet.
				Le but du jeu est simple : le joueur choisit entre trois classes, puis doit terminer un donjon dans lequel il rencontrera différents ennemis. Le système de combat est au tour par tour. Avant un combat, le joueur doit choisir une potion qui lui attribuera un bonus.
			</p>
			),
